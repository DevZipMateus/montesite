...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const intervalDuration = heroImages.length > 0 ? Math.max(4000, Math.min(6000, heroImages.length * 800)) : 5000;
  useEffect(() => {
  ...
  return <section id="home" className="pt-24 pb-16 md:pt-32 md:pb-24 relative overflow-hidden" style={{
            ...
            <FadeIn direction="left">
              ...
              <div className="flex flex-wrap gap-6">
                <Button size="lg" className="rounded-full btn-hover-effect text-lg bg-gradient-to-r from-primary to-indigo-500 hover:from-indigo-500 hover:to-primary transition-all duration-300 px-8 py-6" asChild>
                  <a href="#templates">Escolher Estilo</a>
                </Button>
              </div>
            </FadeIn>
          </div>
              ...
              <div className="relative">
                ...
                <div className="relative overflow-hidden rounded-xl border border-white shadow-2xl bg-white min-h-[300px] flex items-center justify-center">
                  {isLoading ? <div className="flex flex-col items-center gap-3 py-12">
                      ...
                      {heroImages.map((image, index) => <div key={image.id} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'}`}>
                          <img src={image.url} alt={`${image.title} - ${image.type === 'template' ? 'Template' : 'Site em destaque'}`} className="w-full h-full object-contain" loading={index === 0 ? 'eager' : 'lazy'} />
                      ...
                      {heroImages.length > 0 && <img src={heroImages[currentImageIndex]?.url} alt="Sizing placeholder" className="invisible w-full object-contain max-h-[400px]" />}
                      
                      {/* Navigation dots */}
                      {heroImages.length > 1 && <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-2">
                          {heroImages.map((_, index) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`transition-all duration-200 rounded-full ${index === currentImageIndex ? 'bg-primary w-4 h-2' : 'bg-gray-300 hover:bg-gray-400 w-2 h-2'}`} aria-label={`Ver ${heroImages[index]?.title}`} title={`${heroImages[index]?.title} - ${heroImages[index]?.type === 'template' ? 'Template' : 'Site em destaque'}`} />)}
                        </div>}
                    </>}
                </div>
              </div>
