...
const TEMPLATE_IMAGES = ["/imagens/contabilidade-harmonica.png", "/imagens/contabilidade-template.png", "/imagens/easy-financial-solutions.png", "/imagens/conta-connection-hub.png", "/imagens/contador-simplicity.png", "/imagens/contabilify-modern-site.png"];
          ...
          <div className="flex-1">
            <FadeIn direction="left">
              ...
              <div className="flex flex-wrap gap-4">
                <Button size="lg" className="rounded-full btn-hover-effect text-lg bg-gradient-to-r from-primary to-indigo-500 hover:from-indigo-500 hover:to-primary transition-all duration-300" asChild>
                  <a href="#templates">Escolher Template</a>
                </Button>
                <Button variant="outline" size="lg" className="rounded-full text-lg border-primary/20 text-primary hover:bg-primary/10 hover:text-primary hover:border-primary transition-all duration-300" asChild>
            ...
            <FadeIn direction="right" delay={200}>
              <div className="relative">
                ...
                <div className="relative overflow-hidden rounded-xl border border-white shadow-2xl bg-white">
                  {TEMPLATE_IMAGES.map((src, index) => <img key={index} src={src} alt={`Template Preview ${index + 1}`} className={`w-full mx-auto z-10 transition-opacity duration-1000 absolute inset-0 ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'}`} style={{
                  objectFit: 'contain'
                }} />)}
                  
                  <img src={TEMPLATE_IMAGES[currentImageIndex]} alt="Template Preview" className="invisible w-full mx-auto" style={{
                  objectFit: 'contain'
                }} />
                  
                  <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1">
                    {TEMPLATE_IMAGES.map((_, index) => <button key={index} onClick={() => setCurrentImageIndex(index)} className={`w-2 h-2 rounded-full transition-all ${index === currentImageIndex ? 'bg-primary w-4' : 'bg-gray-300 hover:bg-gray-400'}`} aria-label={`Go to slide ${index + 1}`} />)}
                  </div>
                </div>
              </div>
            </FadeIn>
